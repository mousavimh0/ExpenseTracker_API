...
def show_balance(
    ...
    balance = report_service.calculate_balance(db, current_user.id)
    return balance


@report_router.get(
    "/filter/period/{period}/{target_date}", response_model=list[TransactionResponse]
)
def filter_data_by_date(
    ...
    filtered_data = report_service.filtered_date(
        period=period, target_date=target_date, db=db, user_id=current_user.id
    )
...
def get_by_category(categroy: TransactionCategory, db: Session = Depends(get_db)):
    ...
    transactions = report_service.show_transactions_by_column(
        db=db, column=TransactionDB.category, value=categroy
    )
    return transactions
...
def get_by_type(type: TransactionType, db: Session = Depends(get_db)):
    ...
    transactions = report_service.show_transactions_by_column(
        db=db, column=TransactionDB.type, value=type
    )
    return transactions
...
def get_by_date(date: date, db: Session = Depends(get_db)):
    transactions = report_service.show_transactions_by_column(
        db=db, column=TransactionDB.date, value=date
    )
    return transactions
